...
export function showUnlockToast(unlock) {
  ...
  toast.style.cssText = `
    ...
    background:rgba(0,0,0,0.95);border:1px solid var(--neon-green);
    padding:14px 24px;border-radius:4px;z-index:99999;
    font-family:var(--font-cyber);font-size:13px;color:var(--neon-green);
    box-shadow:0 0 20px rgba(57,255,20,0.3);text-align:center;
    animation:slideIn 0.4s ease-out;
  `;
  toast.innerHTML = `
    <div style="font-size:10px;letter-spacing:2px;color:#555;margin-bottom:4px;">UNLOCKED</div>
    <div>${unlock.label}</div>
    <div style="font-size:11px;color:#888;margin-top:4px;font-family:var(--font-main);">${unlock.description.replace('Reach','Reached')}</div>
  `;
